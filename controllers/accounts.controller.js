...
export const depositToAccount = (req, res) => {
  ...
  if (amountToDeposit === undefined) {
    res.status(404).send({ errorStatus: 404, message: 'You must provide the amount to deposit' });
  }
  ...
  if (accounts[userID][accountNumber] === undefined) {
    res.status(404).send({ errorStatus: 404, message: 'Account Not Found!' });
  }

  if (amountToDeposit < 0) {
    res.status(404).send({ errorStatus: 404, message: 'Cash must be a positive number!' });
  }
...
export const updateCredit = (req, res) => {
  ...
  if (newCredit === undefined) {
    res.status(404).send({ errorStatus: 404, message: 'You must provide the new credit amount' });
  }
...
export const withdrawFromAccount = (req, res) => {
  ...
  if (!checkSenderUserAccountBalance(account, amountToWithdraw)) {
    res.status(404).send({ errorStatus: 404, message: 'user doesn\'t have enough credit and cash to withdraw' });
  }
  saveAccountsToDB(accounts);
  res.status(201).send(accounts[userID][accountNumber]);
};